...
UserController.form = async (req, res, next) => {
    ...
    const { username, email, password, password2, name, phone } = req.body;
    console.log(req.body, 'req.body')
    // 1. 비밀번호 일치 불일치 검사
    if (password != password2) {
        return res.status(201).json({ msg: "비밀번호가 일치하지 않습니다." })
    }
    // 2. 비밀번호 유효성 검사
    ...
    if (!regexphone.test(phone)) {
        return res.status(201).json({ msg: "전화번호는 000-0000-0000 형식으로 작성해 주세요" })
    }
    if (!regexemail.test(email)) {
        return res.status(201).json({ msg: "이메일 형식이 아닙니다" })
    }
    ...
    if (!_.isEmpty(search_username)) {
        return res.status(201).json({ msg: "이미 사용중인 아이디 입니다" })
    }
    const search_email = await User.find({ email })
    if (!_.isEmpty(search_email)) {
        return res.status(201).json({ msg: "가입된 이메일 입니다" })
    }
